...
This article, written by Peter Naur in 1985, describes how a programming project should be considered and maintained. For this description, he introduces the term "the project's theory". \\
This theory is formed by the programmers while coding, and can be interpreted as the deepest understanding and philosophy behind the design of the project. \\
The theory can not be documented in the manuals - It requires that a programmer, from the kick-off of the project, has been involved in the coding, and participated in the discussions and choices that have arisen during the project development. \\
In the event that the original team of programmers spilt up, and new programmers, without any knowledge of the project's theory, continues the development, problems will arise.
Due to lack of deeper insight into the structure and design, bad extensions of the program will be created, extensions which in the worst case has already been implemented. \\
This scenario is defined by Naur, as the project's death. The project's death does not mean that the program can not be fixed, also the program and its features still works after its death.
It is first when a demand for new functionality occurs that the problem becomes visible. The quality of the program is sinking because those who develop the program do not have the knowledge needed about the project's theory. A project can only be revived again when a group of programmers redefine the project's theory. \\
In practice, dead projects rarely occur. The original group of programmers can inform the new developers of the project's theory and thus keep the project alive. In the event that the project dies it will probably be cheaper to launch a new project, instead of expanding on the original project. \\
Towards the end of the article Naur argues for why programming should be seen as an academic craftsmanship that can not be assimilated to a craftsman or construction worker. Here Naur refers to the above definition of project theory. He considers a craftsman as a kind of worker that can be substituted with any other craftsman. This could be transferred directly to programming if it were not for the project's theory. Because of the project's theory and a programmers knowledge of this, it is necessary to hire the programmer permanently, to avoid that the program's theory disappears and the program thus die. \\
In terms of our project, we have developed a theory, however without being aware of it during the process. If a programmer outside of the project should implement more functionality to our product, it would not necessarily be performed in the same way as we in the group would have implemented it. However, the size of the project, and our decision to work with PHP and mySQL should be considered. In addition, the project is not larger at this point in the process, than an experienced programmer could familiarize themselves with the code and implement functionality that was at least as good as we can implement.
\subsection*{Christensen et al. (1998) The M.A.D Experience}
The MAD Experience is a paper from 1998 and deals with the reflection of a project between a research group from Aarhus University and a large shipping company to develop a prototype of a global customer service system. While no one from the Danish research group had worked together before, and neither had any experience in shipping, it proved possible for them to make a prototype that met and actually exceeded expectations for the shipping company. \\
During the project, they discovered that the analysis is more than just finding nouns and verbs, design is more than just filling in the details of the object-oriented analysis model and implementation is more than just translating design models into code. \\
The roles in the collaboration was as follows:
\begin{itemize}
\item The ethnographer - which had the focus and experience with the practical within customer service and related areas.
\item The participatory designer - which focused on the design of future practical and technological assistance to customers.
\item OO developer - which focused on the development of the object model and implementation.
\end{itemize}
The ethnographers focus is to provide a reference to the real world and organization. He bases his design on a practical understanding and builds on the system to be implemented. He is expected by his practical experience, to adapt the system to fit into the activities that will be assembled in. He must visualize what kind of work the system must help perform by directly observing and portraying the work of the customer. He should do the job at the customer understandable for the programmers, so they have clear guidelines and a clear goal to work towards. \\
The ethnographers job is to identify a problem-solution area. However, he is not capable of identifying a design solution which is largely identified through prototyping. \\
The participatory designer emphasizes on the importance of the creative involvement of potential users, managers and other affected in the design process. He does this both because of the moral perspective - the user is the one who must live with the design later - and because of a practical perspective, namely that they are the best test-subjects for a system designed for exactly those. \\
Design is therefore something to cooperate on, and should not only include end users, but also people with other skills and ideas. It often takes place in a kind of workshops where you blend in the competencies included in the discussion to a specific problem. \\
The paper gives an example of how the first two interactions with the customer work. Four developers and two business representatives went to Singapore where they, among other things, presented their prototype, saw the process of a normal working-day and discussed how they could improve their design.\\
OO developer contributes with an integration perspective on the analysis, design and implementation. However, there should be a balance between modelling and programming, otherwise it will be difficult to handle the implementation. \\
In this project the OO developers was involved from the beginning and the modelling was purposely an integral part of the many prototypes iterations. \\
The text also points out the importance of having a working prototype early in the process, even if this reflects a minimum of design. It is important, so you have something to show, test and get feedback on.\\
The research group finally concludes why they feel their project is so successful. Here they go, among other things into how important it is to focus on the practical situation in which the design should be adapted, but also how it is important that analysis, design and implementation is a function of each other and an integral part of the project. In addition, it is important to have a variety of perspectives on system development, which in their project comes from the different roles as described previously.
...
\quad \quad \quad \text{2. \texttt{Props 2.0} receives the query, the MySQL database} \\ \quad \quad \quad \quad \text{returns the matching data and \texttt{Props 2.0} displays it}\\
\text{3. The \texttt{Employee} can now choose a prop or production.} \\
\end{array} \\
\hline
\textit{Entry condition} & \text{The \texttt{Employee} must be logged into the Theatre's Wi-Fi} \\
\hline
...
\end{array} \\
\hline
\textit{Entry condition} & \text{This use case extends the  \texttt{PropSearch} use case OR}\\ & 
\text{Can be initiated from the frontpage}\\
\hline
...
\text{This use case extends the \texttt{ProductionSearch} use case OR}\\ & 
\text{Can be initiated from the frontpage}\\
\hline
...
We've chosen the use case of \texttt{AddProb}, \texttt{EditProject} and \texttt{Search} thus we think these are the main use cases of our webapplication.\\\\
\textbf{addProp sequence diagram}\\
Brugeren trykker på \textit{tilføj prop} linket, og der bliver oprettet et kontrol element, hvilket vi bruger som en abstaktion over den bagvedliggende funktionallitet. Linket kan nåes fra alle undersider. Kontrol elementet opretter en form med felter til alle de informationer man kan udfylde om en prop. Brugeren udfylder de relevante felter og trykker submit. Formen sender en besked om dette til kontrol elementet som prøver at opdatere databasen. Databasen sender en statusreport tilbage og kontrolelementer viser en statusmeddelelse til brugeren og hvis prop'en er blevet oprettet, så vises den nyoprettede prop.
Derefter fjerner kontrol elementet 'tilføj prop' formen.
\\\\
\includegraphics[scale=0.6]{sequenceDiagram_addProp.png}
\\\\\\
\textbf{editProp sequence diagram}\\
Brugeren trykker på \textit{rediger prop} knappen, og der bliver oprettet et kontrol element. Knappen kan nåes fra show prop siden. Kontrol elementet opretter en form med felter til alle de informationer man kan udfylde om en prop. Formen er fyldt med de informationer der i forvejen er udfyldt om den givne prop. Brugeren ændrer de relevante felter og trykker submit. Formen sender en besked om dette til kontrol elementet som prøver at opdatere databasen. Databasen sender en statusreport tilbage og kontrolelementer viser en statusmeddelelse til brugeren og hvis prop'en er blevet oprettet, så vises den ændrede prop.
Derefter fjerner kontrol elementet 'rediger produktions' formen.
\\\\
\includegraphics[scale=0.6]{sequenceDiagram_editProduction.png}\\\\\\
\textbf{searchProp sequence diagram}\\
Brugeren fylder enten søgefeltet, de avangcerede søgefelter eller en kombination af de to, trykker på submit knappen, og der bliver oprettet et kontrol element. søgefelterne kan nåes fra alle undersider. Kontrol elementet anmoder databasen om søgeresultaterne og databasen sender søgeresultaterne tilbage. 
Kontrol elementet opretter to tabeller med props søgeresultater og productions søgeresultater. Brugeren vælger en prop. kontrol elementet får en besked om at en prop er valgt og opretter en prop side, altså en side med informationer om den givne prop. Derefter fjerner kontrol elementet søgetabellerne.
\\\\
\includegraphics[scale=0.6]{sequenceDiagram_searchProp.png}\\
